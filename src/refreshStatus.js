...
const refreshStatus =
	...
	async () => {
		...
		const formattedProcessesUB = selectedProcesses.map(p =>
		...
		formattedProcessesUB.forEach(fp => {
			if (options.bumpStateProcessesBy && fp.state)
				stateArray.push({ fp, state: fp.state() });
		});

		await Promise.all(stateArray.map(s => s.state)).then(states =>
		...
		if (buttons.length === 1 && options.extraButton)
